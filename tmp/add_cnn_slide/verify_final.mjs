...
const OUT = "C:\\docs\\from_linear_regression_to_gpt\\tmp\\add_cnn_slide\\verified\\final-render";
...
for (let index = 0; index < presentation.slides.items.length; index += 1) {
  ...
  await writeBlob(`${OUT}\\${stem}.png`, await slide.export({ format: "png", scale: index === 18 ? 2 : 1 }));
  await fs.writeFile(`${OUT}\\layouts\\${stem}.layout.json`, await (await slide.export({ format: "layout" })).text(), "utf8");
}
const notes = presentation.slides.getItem(18).speakerNotes.textFrame.text;
console.log(`slides=${presentation.slides.items.length}`);
console.log(`cnnSourcesPresent=${String(notes).includes("developersbreach.com")}`);
